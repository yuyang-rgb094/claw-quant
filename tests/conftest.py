...
def _test_databases():
    ...
    # Insert minimal test data so queries return non-empty results
    _seed_test_data(new_paths)
...
def _seed_test_data(paths: dict) -> None:
    """Insert minimal but valid test data into all databases."""
    import sqlite3

    # ── carhart_results.db ──
    conn = sqlite3.connect(str(paths["carhart"]))
    conn.executescript("""
        ...
        VALUES
            ('A_share', '2024-01-01', '2024-06-30',
             0.025, 2.1, 0.035, 0.82, 0.80, 0.015, 0.45, 120, '2024-07-01'),
            ('A_share', '2024-02-01', '2024-07-31',
    ...
    # ── factor_ic.db ──
    conn = sqlite3.connect(str(paths["factor_ic"]))
    conn.executescript("""
        INSERT INTO ic_series (factor, date, forward_days, ic)
        VALUES
    ...
    # ── crowding.db ──
    conn = sqlite3.connect(str(paths["crowding"]))
    conn.executescript("""
        INSERT INTO crowding
        ...
        INSERT INTO crowding_history (factor, date, crowding_score)
        VALUES
            ('MKT', '2024-06-01', 0.30),
            ('MKT', '2024-06-15', 0.33),
            ('MKT', '2024-06-28', 0.35),
    ...
    # ── cffex_positions.db ──
    conn = sqlite3.connect(str(paths["cffex"]))
    conn.executescript("""
        INSERT INTO signals
